...
map<string, int> readAndSendDatabase(const char* fileName, int serverPort) {
    ...
    map<string, int> historyDatabase;
    ...
    while (getline (myfile, data)) {
        ...
        if (getline(ss, bookCode, ',') && ss >> amount) {
            historyDatabase[bookCode] = amount;
        } 
        else {
    ...
    if (udpSocket == -1) {
        cout << "Error creating UDP socket in serverHistory" << endl;
    }

    sockaddr_in serverMainAddress;
    serverMainAddress.sin_family = AF_INET;
    serverMainAddress.sin_port = htons(43209); 
...
void UDPConnection(int port, map<string, int> database) {
    ...
    while (true) { // While loop to continuously listen for new requests.
        ...
        if (bytesReceived != -1) {
            ...
            // Check if the book code is in the serverH database. Used explanation from Chat GPT to write the loop to search for bookcode and its respective amount.
            ...
            if (bookCodeIt != database.end() && bookCodeIt->second > 0) { // Book code found and more than 0 left
                ...
                cout << "Zero left in ServerH's database. Sending reply to ServerM" << endl;
...
int main() {
    ...
    database = readAndSendDatabase("history.txt", 43209);
    UDPConnection(43209, database);
    return 0;
}