...
map<string, int> readAndSendDatabase(const char* fileName, int serverPort) {
    ...
    map<string, int> literatureDatabase;
    ...
    while (getline (myfile, data)) {
        ...
        if (getline(ss, bookCode, ',') && ss >> amount) {
            literatureDatabase[bookCode] = amount;
        } 
        else {
    ...
    if (udpSocket == -1) {
        cout << "Error creating UDP socket in serverLiterature" << endl;
    }

    sockaddr_in serverMainAddress;
    serverMainAddress.sin_family = AF_INET;
    serverMainAddress.sin_port = htons(42209); 
    ...
    for (const auto& entry : literatureDatabase) {
...
void UDPConnection(int port, map<string, int> database) {
    ...
    while (true) { // While loop to continuously listen for new requests.
        ...
        if (bytesReceived != -1) {
            ...
            // Check if the book code is in the serverL database. Used explanation from Chat GPT to write the loop to search for bookcode and its respective amount.
            ...
            if (bookCodeIt != database.end() && bookCodeIt->second > 0) { // Book code found and more than 0 left
                ...
                cout << "Zero left in ServerL's database. Sending reply to ServerM" << endl;
...
int main() {
    ...
    database = readAndSendDatabase("literature.txt", 42209);
    UDPConnection(42209, database);
    return 0;
}